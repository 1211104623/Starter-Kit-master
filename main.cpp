...
// balqis -> features has to be comment because balqis didn't send us her SDL2 folder.
/*void changearrow()
{
...
{
    if (board[colm_ - row][col - 1] != '.') //to check if the position is not a trail.
    {
...
void GameBoard::resetAlien(int& alienRow, int& alienCol)
{
    int newRow;
    int newCol;
    do //random position on the board game
    {
        newRow = rand() % row_ + 1;
        newCol = rand() % colm_ + 1;
    } 
...
// uzma
void movezombie(int rows, int colms, int zombies) // NOT YET DONE
{
    GameBoard gameboard;
    char entah;
    char moves[] = {'^', 'v', '<', '>'};
    ...
    cout << " ERROR! : Now is zombie's turn. But, the zombie is not yet on the gameboard. Please proceed to the next step. " << endl;
    gameboard.commands(rows, colms, zombies);
}
...
char GameBoard::commands(int rows, int colms, int zombies)
{
    ...
    else if (commandarrow == usercommand)
    {
        cout << "This feature is not done yet. Please continue to the next step. \n";
        //changearrow();
    ...
    else if (commandsave == usercommand) // NOT YET DONE
    {
        ...
        cout << "This feature is not done yet. \n"; 
        commands(rows, colms, zombies);
    }
    ...
    {
        cout << "PLEASE ENTER THE RIGHT COMMAND! \n" ;
        commands(rows, colms, zombies);
    }
    return 0;
}
